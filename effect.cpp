...
HRESULT effect::init(image * effectImage, int frameW, int frameH, int fps, float elapsedTime, bool isAlpha, BYTE alpha)
{
	//만약에 이미지가 없으면 리턴
	if (!effectImage) return E_FAIL;
...
void effect::render()
{
	...
	//Effect 애니메이션 그려주자
	if (_isAlpha) _effectImage->aniRender(getMemDC(), _x, _y, _effectAnimation, _alpha);
	else _effectImage->aniRender(getMemDC(), _x, _y, _effectAnimation);
}
